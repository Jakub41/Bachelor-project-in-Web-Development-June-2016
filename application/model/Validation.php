...
class Validation {
     ...
     * @return bool
     */
	 public function isEditUnique($tablename,$fieldnameone,$valueone,$fieldnametwo,$valuetwo,$type)
     {

	    $sql=  "SELECT count(*) as total  FROM ".$tablename." WHERE ".$fieldnameone."=:valueone and ".$fieldnametwo."!=:valuetwo";
	    ...
	    $isunique =$query->fetch();
	    if($isunique["total"]>0)
		{
		$this->addError("unique", $valueone, $type);
		}
	 }
	 public function isUnique($tablename,$fieldnameone,$valueone,$type)
     {

	    $sql=  "SELECT count(*) as total  FROM ".$tablename." WHERE ".$fieldnameone."=:valueone";
	 	$query = $this->db->prepare($sql);
	    $parameters = array(
            ':valueone' =>$valueone);
		$query->execute($parameters);
